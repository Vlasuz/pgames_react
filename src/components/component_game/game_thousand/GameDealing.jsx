...
import {gameCards, gameCardsDealing, gameCardsLft, gameCardsRht} from "../../../redux/actions";
...
const GameDealing = ({setStepOfGame}) => {
    ...
    const [allCards, setAllCards] = useState([
        {
            status: false,
            card: {id: 1, card_type: 9, card_suit: 1},
        },
        {
            status: false,
            card: {id: 2, card_type: 9, card_suit: 2},
        },
        {
            status: false,
            card: {id: 3, card_type: 9, card_suit: 3},
        },
        {
            status: false,
            card: {id: 4, card_type: 9, card_suit: 4},
        },
        {
            status: false,
            card: {id: 5, card_type: 10, card_suit: 1},
        },
        {
            status: false,
            card: {id: 6, card_type: 10, card_suit: 2},
        },
        {
            status: false,
            card: {id: 7, card_type: 10, card_suit: 3},
        },
        {
            status: false,
            card: {id: 8, card_type: 10, card_suit: 4},
        },
        {
            status: false,
            card: {id: 9, card_type: 11, card_suit: 1},
        },
        {
            status: false,
            card: {id: 10, card_type: 11, card_suit: 2},
        },
        {
            status: false,
            card: {id: 11, card_type: 11, card_suit: 3},
        },
        {
            status: false,
            card: {id: 12, card_type: 11, card_suit: 4},
        },
        {
            status: false,
            card: {id: 13, card_type: 12, card_suit: 1},
        },
        {
            status: false,
            card: {id: 14, card_type: 12, card_suit: 2},
        },
        {
            status: false,
            card: {id: 15, card_type: 12, card_suit: 3},
        },
        {
            status: false,
            card: {id: 16, card_type: 12, card_suit: 4},
        },
        {
            status: false,
            card: {id: 17, card_type: 13, card_suit: 1},
        },
        {
            status: false,
            card: {id: 18, card_type: 13, card_suit: 2},
        },
        {
            status: false,
            card: {id: 19, card_type: 13, card_suit: 3},
        },
        {
            status: false,
            card: {id: 20, card_type: 13, card_suit: 4},
        },
        {
            status: false,
            card: {id: 21, card_type: 14, card_suit: 1},
        },
        {
            status: false,
            card: {id: 22, card_type: 14, card_suit: 2},
        },
        {
            status: false,
            card: {id: 23, card_type: 14, card_suit: 3},
        },
        {
            status: false,
            card: {id: 24, card_type: 14, card_suit: 4},
        },
    ]);
    const [cardsForMe, setCardsForMe] = useState([])
    const [cardsForLft, setCardsForLft] = useState([])
    const [cardsForRht, setCardsForRht] = useState([])
    const [cardsForThree, setCardsForThree] = useState([])
    ...
    useEffect(() => {
        ...
        if (cardsForMe.length < 7 || cardsForLft.length < 7 || cardsForRht.length < 7 || cardsForThree.length < 3) {

            let timer = setInterval(() => {

                let rand = Math.floor(Math.random() * allCards.length)

                if (!allCards[rand].status) {
                    if (cardsForMe.length < 7) {
                        allCards[rand].status = true
                        setCardsForMe(arr => [...arr, allCards[rand].card])
                    }
                }
                if (!allCards[rand].status) {
                    if (cardsForLft.length < 7) {
                        allCards[rand].status = true
                        setCardsForLft(arr => [...arr, allCards[rand].card])
                    }
                }
                if (!allCards[rand].status) {
                    if (cardsForRht.length < 7) {
                        allCards[rand].status = true
                        setCardsForRht(arr => [...arr, allCards[rand].card])
                    }
                }
                if (!allCards[rand].status) {
                    if (cardsForThree.length < 3) {
                        allCards[rand].status = true
                        setCardsForThree(arr => [...arr, allCards[rand].card])
                    }
                }
            ...
            // HERE GAME AFTER DEALING CARDS
            dispatch(gameCards(cardsForMe))
            dispatch(gameCardsLft(cardsForLft))
            dispatch(gameCardsRht(cardsForRht))
            dispatch(gameCardsDealing(cardsForThree))
    ...
    }, [cardsForMe, cardsForLft, cardsForRht, cardsForThree])
    ...
    useEffect(() => {
        ...
        if (lftAnswerIs && rhtAnswerIs) {
            ...
            dispatch(gameCards(cardsForThree))
                            ...
                            <div className="one-thousand__grid--item">
                                <div className="one-thousand__back-cards">
                                    <ul className="one-thousand__back-cards--list">
                                        ...
                                        {
                                            cardsForThree.map(card =>
                                                <li key={card.id}
                                                    className={"one-thousand__back-cards--item" + (isShowCards ? " _active" : "")}>
                                ...
                                <div className="one-thousand__user game__user">
                                    ...
                                    <div
                                        ...
                                        <ul className="game-user-cards__list">
                                            ...
                                            {
                                                cardsForMe.sort((a, b) => a.card_suit - b.card_suit).map((item, itemNum) =>
                                                    <li key={itemNum}
