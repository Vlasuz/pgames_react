...
import {gameCardsDelete, gameCardsLft, gameCardsRht} from "../../../redux/actions";
...
const GameDealing = ({setStepOfGame}) => {
    ...
    const giveSomeone = () => {
        ...
        if(!isGiveLeft){
            ...
            dispatch(gameCardsRht(cards.filter(item => {if(+item.id === +cardSelect) return item} )))
            dispatch(gameCardsDelete(cardSelect))

        } else {
            document.querySelector('.game-user-cards__item._active').style.transform = 'translate(-100px, -50px) scale(0)'

            dispatch(gameCardsLft(cards.filter(item => {if(+item.id === +cardSelect) return item} )))
            dispatch(gameCardsDelete(cardSelect))
